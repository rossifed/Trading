...
BEGIN
    BEGIN TRY
        -- Define the test server nam and database name
        DECLARE @SchemaName NVARCHAR(128)= 'ord'
        ...
        -- Step 1: Drop all foreign key constraints in the specified schema
        ...
        -- Step 2: Disable system versioning on all temporal tables and drop them along with their history tables in the specified schema
        ...
        WHERE t.temporal_type = 2 AND s.name = @SchemaName; -- Temporal table
        EXEC sp_executesql @dropTemporalTables;

        -- Step 3: Drop all remaining tables in the specified schema
        DECLARE @dropTables NVARCHAR(MAX) = N'';
        SELECT @dropTables += 'DROP TABLE ' + QUOTENAME(s.name) + '.' + QUOTENAME(t.name) + ';' + CHAR(13)
        FROM sys.tables t
        JOIN sys.schemas s ON t.schema_id = s.schema_id
        WHERE t.temporal_type <> 2 AND s.name = @SchemaName; -- Non-temporal table
