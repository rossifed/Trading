...
{
    internal class CreateOrderResponseParser : IMessageParser<CreateOrderResponse>
    {
        public CreateOrderResponse Parse(Message msg)
        {
            int emsx_sequence = msg.GetElementAsInt32(EMSX_SEQUENCE);
            String message = msg.GetElementAsString(MESSAGE);
            return new CreateOrderResponse(emsx_sequence, message);
        }
    }
}
